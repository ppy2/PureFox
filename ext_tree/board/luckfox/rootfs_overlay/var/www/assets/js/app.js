$(document).ready(function () {
    ...
    // Упрощенная функция переключения ALSA с автоматической перезагрузкой
    function switchAlsa(cardType) {
        // UI уже обновлен пользователем (toggle switch), показываем спиннер
        ...
        $('.spinner-text').text(switchingTexts[currentLang] || switchingTexts['en']);
        ...
        $.ajax({
            ...
            timeout: 5000,
            success: function(response) {
                // USB/I2S переключение без перезагрузки - просто скрываем спиннер
                $('.spinner-overlay').removeClass('show');
                // Даем время сервисам полностью перезапуститься перед разблокировкой
                setTimeout(function() {
            ...
            error: function(xhr, status, error) {
                // Ошибка переключения - откатываем состояние
                $('.spinner-overlay').removeClass('show');
                const oppositeState = (cardType === 'usb') ? 'i2s' : 'usb';
                updateAlsaUI(oppositeState);
                isAlsaSwitching = false;
                console.error('ALSA switching error:', error);
            }
        });
    }
    ...
    let rebootCheckAttempts = 0;
    const maxRebootAttempts = 30; // максимум 60 секунд ожидания
    
    function checkConnectionAfterReboot() {
        rebootCheckAttempts++;
        
        // После 30 попыток (60 сек) останавливаем проверки
        if (rebootCheckAttempts > maxRebootAttempts) {
            $('.spinner-overlay').removeClass('show');
            alert('Система не отвечает. Проверьте подключение и обновите страницу вручную.');
            return;
        }
        ...
        $.ajax({
            ...
            success: function() {
                ...
                rebootCheckAttempts = 0; // сбрасываем счетчик
                location.reload();
            },
            error: function() {
                // Адаптивная задержка: быстрее в первые попытки
                const delay = rebootCheckAttempts < 10 ? 1000 : 2000;
                setTimeout(checkConnectionAfterReboot, delay);
            }
        });
    }
