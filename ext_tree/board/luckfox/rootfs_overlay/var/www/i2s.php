...
<head>
    ...
    <script>
        ...
        function checkConnection() {
            ...
            fetch('status_fast.php', { timeout: 5000 })
                .then(response => {
                    if (response.ok) {
                        $('.spinner-overlay').removeClass('show');
                        location.reload();
                    } else if (rebootCheckAttempts > maxRebootAttempts) {
                        $('.spinner-overlay').removeClass('show');
                        alert('Система не отвечает после множества попыток...');
                    } else {
