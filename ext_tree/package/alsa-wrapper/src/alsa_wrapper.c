...
static void scale_audio_data(void *buffer, snd_pcm_uframes_t frames, 
    ...
    // Поддерживаем только основные форматы для совместимости
    switch (format) {
...
int snd_pcm_hw_params_can_pause(const snd_pcm_hw_params_t *params) {
    (void)params; // Подавляем предупреждение unused parameter
    return 1;
}
...
// ===== ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ ВНЕШНЕГО УПРАВЛЕНИЯ =====
